...
static inline void
fill_payload(struct payload *payload_data) {
    // payload_data->flow_size = flow->flow_size;
    // payload_data->pkt_seq = flow->pkt_seq;
    payload_data->timestamp = rte_rdtsc(); //shoule change later
    // flow->pkt_seq++;
}
...
int main(int argc, char *argv[])
{
    ...
    fprintf(fp, "%d,%ld,%d,%d,%ld,%ld,%lf,%lf,%lf,%lf\r\n", \
            n_lcores, timetag.tv_sec, FLOW_NUM, PKT_LEN, total_tx_pkt_num, total_rx_pkt_num, total_tx_pps, total_tx_bps, total_rx_pps, total_rx_bps);
    fclose(fp);
    ...
    for (i = 0;i<MAX_RECORD_COUNT;i++){
        double tx_pps_p = 0.0, tx_bps_p = 0.0;
        double rx_pps_p = 0.0, rx_bps_p = 0.0;
        ...
        fprintf(fp, "%d,%ld,%d,%d,%d,%lf,%lf,%lf,%lf\r\n", \
                n_lcores, timetag.tv_sec, FLOW_NUM, PKT_LEN, i, tx_pps_p, tx_bps_p, rx_pps_p, rx_bps_p);
    }
    fclose(fp);
